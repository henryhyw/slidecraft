...
document.addEventListener('click', event => attempt(async () => {
  ...
  if (button.id === 'install-sam') { if ($('#preview-dialog').open) $('#preview-dialog').close(); return editor('Install SAM', '<p>Improve irregular outlines in your slides. Installation downloads a model and supporting tools and may use several gigabytes.</p><p>If installation cannot finish, you can continue creating slides with OpenCV.</p>', async () => {
    state.samInstall = await api('/api/capability/sam/install', {}); renderHealth(); openCapability('SAM'); await pollSamInstall();
...
async function pollSamInstall() {
  ...
  else if (status.state === 'complete') { await loadSystem(); if ($('#preview-dialog').open && $('#preview-title').textContent === 'SAM') openCapability('SAM'); toast('SAM is ready'); }
  else if (['failed', 'interrupted'].includes(status.state)) toast(status.message || 'SAM installation could not finish. OpenCV remains available.');
}
